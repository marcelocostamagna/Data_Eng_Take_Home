...
def read_save_json():
    ...
    response = requests.get('http://api.aviationstack.com/v1/flights?access_key=' +
                            ACCESS_KEY +
...
def create_flights_data(file_path):
    ...
    data[['departure_airport', 'departure_timezone']] = \
        pd.json_normalize(df_raw['departure'])[['airport', 'timezone']]
    data[['arrival_airport', 'arrival_timezone', 'arrival_terminal']] = \
        pd.json_normalize(df_raw['arrival'])[['airport', 'timezone', 'terminal']]
